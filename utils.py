...
def get_duo_info(account: int, show_timestamp: bool = False):
    ...
    if response.status_code == 200:
        if DEBUG:
            print(f"{f'{current_time()} ' if show_timestamp else ''}[bold magenta][DEBUG][/] Retrieved Duolingo info for user {config['accounts'][account]['username']}")
        return response.json()
    else:
        if DEBUG:
            print(
                f"{f'{current_time()} ' if show_timestamp else ''}[bold magenta][DEBUG][/] Failed to retrieve Duolingo info for user {config['accounts'][account]['username']}\n"
                f"{f'{current_time()} ' if show_timestamp else ''}[bold magenta][DEBUG][/] Status code {response.status_code}\n"
                f"{f'{current_time()} ' if show_timestamp else ''}[bold magenta][DEBUG][/] Content: {response.text}"
            )
        return None

def fetch_username_and_id(token: str):
